 ...
 * `components/enrollment/student-details-form.tsx` collected.
 *
 * Round-2 audit: `dateOfBirth` is destructured by `POST /api/enrollments` and
 * written to `Student.dateOfBirth` (route.ts:110), which is the field the
 * birthday-email cron reads. No screen collected it, so every student created
 * through this wizard had a null birthday and could never receive one. It is
 * optional on the server (`dateOfBirth ? new Date(...) : null`) and stays
 * optional here.
 */
    ...
    <div>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <SelectField
          ...
          placeholder="Select a time that suits you"
      ...
      <NoteBlock>
        We use the date of birth to send a birthday note, and nothing else. If the student is under
        18, add a guardian's contact below so we know who to reach about lessons and payments.
      </NoteBlock>

      <div className="mt-7 border-t border-bfx-hair pt-7">
        <SubHeading
          title="Guardian details"
          sub="Optional. Add these if the student is under 18, or if someone else handles the lessons and payments."
        />
        <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <TextField
            id="guardianName"
